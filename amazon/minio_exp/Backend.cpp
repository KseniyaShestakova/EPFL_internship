...
struct BackendObject {
    ...
    BackendObject(const std::string& bucket, const std::string& object) :
        bucket(bucket), object(object), refCount(0) {}
};
...
int main() {
    ...
    std::cout << exists(bd, "tmp-bucket-5", "strange_object.txt");
    ...
    create_without_check(bd, "tmp-bucket-5", "foo.txt", &bo);
